...
<body class="font-sans antialiased bg-gray-100 text-gray-900 dark:bg-gray-900 dark:text-white">
    ...
    <div class="min-h-screen bg-gray-100 dark:bg-gray-900">
        <!-- Investor Navigation (similar to staff navigation but simplified) -->
        <nav x-data="{ open: false }" class="bg-brand-darker border-b border-brand-dark dark:border-brand-dark">
                ...
                <div class="flex justify-between h-16">
                    <div class="flex">
                        ...
                        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex text-white">
                            <x-nav-link :href="route('investor.dashboard')" :active="request()->routeIs('investor.dashboard')" class="text-white hover:text-brand-accent">
                                {{ __('Dashboard') }}
                            </x-nav-link>
                        </div>

                        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex text-white">
                            <x-nav-link :href="route('investor.documents')" :active="request()->routeIs('investor.documents')" class="text-white hover:text-brand-accent">
                                {{ __('Documents') }}
                            </x-nav-link>
                        </div>
                    </div>
                    ...
                    <div class="hidden sm:flex sm:items-center sm:ms-6">
                        <x-dropdown align="right" width="48">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link :href="route('investor.profile')">
                                    {{ __('Profile') }}
                                </x-dropdown-link>
                                ...
                                <form method="POST" action="{{ route('investor.logout') }}">
                                    ...
                                    <x-dropdown-link :href="route('investor.logout')"
            ...
            <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
                <div class="pt-2 pb-3 space-y-1">
                    <x-responsive-nav-link :href="route('investor.dashboard')" :active="request()->routeIs('investor.dashboard')">
                        {{ __('Dashboard') }}
                    </x-responsive-nav-link>
                    <x-responsive-nav-link :href="route('investor.documents')" :active="request()->routeIs('investor.documents')">
                        {{ __('Documents') }}
                    </x-responsive-nav-link>
                </div>
                ...
                <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
                    ...
                    <div class="mt-3 space-y-1">
                        <x-responsive-nav-link :href="route('investor.profile')">
                            {{ __('Profile') }}
                        </x-responsive-nav-link>
                        ...
                        <form method="POST" action="{{ route('investor.logout') }}">
                            ...
                            <x-responsive-nav-link :href="route('investor.logout')"
                                    onclick="event.preventDefault();
                                                this.closest('form').submit();">
                                {{ __('Log Out') }}
                            </x-responsive-nav-link>
