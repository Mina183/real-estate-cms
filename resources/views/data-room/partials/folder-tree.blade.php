{{-- resources/views/data-room/partials/folder-tree.blade.php --}}
{{-- Koristi se sa: @include('data-room.partials.folder-tree', ['folder' => $folder]) --}}

<div class="mb-1">
    {{-- Parent Folder Row --}}
    <div class="folder-row" onclick="toggleFolder('{{ $folder->id }}')">
        <svg id="chevron-{{ $folder->id }}" class="w-4 h-4 chevron {{ $folder->children->count() || $folder->documents->count() ? '' : 'opacity-0' }}"
        ...
        <span class="badge badge-{{ $folder->access_level }}">
            {{ ucfirst(str_replace('_', ' ', $folder->access_level)) }}
        </span>
    </div>

    {{-- Folder Contents --}}
    <div id="folder-{{ $folder->id }}" class="collapsible hidden ml-4 border-l border-gray-100 pl-2 mt-0.5 mb-1">
        ...
        @foreach($folder->children as $child)
            <div class="mb-0.5">
                {{-- Subfolder Row --}}
                <div class="subfolder-row" onclick="toggleFolder('{{ $child->id }}')">
                    <svg id="chevron-{{ $child->id }}" class="w-3.5 h-3.5 chevron {{ $child->documents->count() ? '' : 'opacity-0' }}"
                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                    </svg>
                    ...
                    <span class="badge badge-{{ $child->access_level }}">
        ...
        @if($folder->documents->isEmpty() && $folder->children->isEmpty())
            <div class="py-3 pl-4 text-xs text-gray-400 italic">Empty folder</div>
        @endif
