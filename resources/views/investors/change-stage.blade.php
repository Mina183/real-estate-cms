    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-6">
                    ...
                    @if ($errors->any())
                        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-6">
                            ...
                            @if(!$errors->has('requirements') && !$errors->has('error'))
                                <p>{{ $errors->first() }}</p>
                            @endif
                    ...
                    <form action="{{ route('investors.change-stage', $investor) }}" method="POST">
                        ...
                        <div class="space-y-6">
                            ...
                            <!-- Stage Requirements Info (populated by JS) -->
                            ...
                            <!-- Current Investor Gate Status -->
                            <div class="border-t border-gray-200 pt-4">
                                <h5 class="font-semibold text-gray-700 mb-3">Current Gate Status:</h5>
                                <div class="grid grid-cols-2 gap-2 text-xs">

                                    {{-- Stage 2: Eligibility gates --}}
                                    <div class="col-span-2 text-gray-400 font-medium uppercase tracking-wide mt-1">Stage 2 — Eligibility</div>

                                    <div class="flex items-center">
                                        <span class="{{ ($investor->target_commitment_amount ?? 0) >= 1000000 ? 'text-green-600' : 'text-red-500' }} mr-2 font-bold">
                                            {{ ($investor->target_commitment_amount ?? 0) >= 1000000 ? '✓' : '✗' }}
                                        </span>
                                        <span>Target Commitment ≥$1M ({{ $investor->currency }} {{ number_format($investor->target_commitment_amount ?? 0) }})</span>
                                    </div>

                                    <div class="flex items-center">
                                        <span class="{{ $investor->is_professional_client ? 'text-green-600' : 'text-red-500' }} mr-2 font-bold">
                                            {{ $investor->is_professional_client ? '✓' : '✗' }}
                                        </span>
                                        <span>Professional Client confirmed</span>
                                    </div>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <span>Subscription Agreement signed</span>
                                    </div>

                                    <div class="flex items-center">
                                        <span class="{{ ($investor->final_commitment_amount ?? 0) > 0 ? 'text-green-600' : 'text-red-500' }} mr-2 font-bold">
                                            {{ ($investor->final_commitment_amount ?? 0) > 0 ? '✓' : '✗' }}
                                        </span>
                                    ...
                                    <div class="flex items-center">
                                        <span class="{{ in_array($investor->kyc_status, ['in_progress','submitted','under_review','complete']) ? 'text-green-600' : 'text-red-500' }} mr-2 font-bold">
                                            {{ in_array($investor->kyc_status, ['in_progress','submitted','under_review','complete']) ? '✓' : '✗' }}
                                        </span>
                                    ...
                                    <div class="flex items-center">
                                        <span class="{{ $investor->sanctions_check_passed ? 'text-green-600' : 'text-red-500' }} mr-2 font-bold">
                                            {{ $investor->sanctions_check_passed ? '✓' : '✗' }}
                                        </span>
                                        <span>Sanctions Check passed</span>
                                    </div>

                                    <div class="flex items-center">
                                        <span class="{{ $investor->commitment_letter_signed ? 'text-green-600' : 'text-red-500' }} mr-2 font-bold">
                                            {{ $investor->commitment_letter_signed ? '✓' : '✗' }}
                                        </span>
                                    ...
                                    <div class="flex items-center">
                                        <span class="{{ ($investor->funded_amount ?? 0) > 0 ? 'text-green-600' : 'text-red-500' }} mr-2 font-bold">
                                            {{ ($investor->funded_amount ?? 0) > 0 ? '✓' : '✗' }}
                                        </span>
                                        <span>Funded Amount entered ({{ $investor->currency }} {{ number_format($investor->funded_amount ?? 0) }})</span>
                                    </div>
                            ...
                            <div>
                                ...
                                <textarea name="reason" id="reason" rows="4"
                                          placeholder="e.g., KYC documents reviewed and approved, Subscription agreement received, etc."
                                ...
                                <p class="mt-2 text-sm text-gray-500">
                                    Provide context for this stage change (will be logged in the audit trail)
                                </p>
                            </div>
            ...
            <div class="mt-6 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Stage Progression</h3>
    ...
    document.getElementById('new_stage').addEventListener('change', function() {
        ...
        if (stageValue && stageRequirements[stageValue]) {
            ...
            infoDiv.innerHTML = `
                <div class="space-y-3">
                    <div>
                        <p class="font-semibold text-gray-700 mb-2">Missing requirements:</p>
                        ${requirementsHtml}
                    </div>
                    <div>
                        <p class="font-semibold text-gray-700 mb-1">Automatic actions on move:</p>
                        <p class="text-sm text-blue-700">${info.automation || 'None'}</p>
                    </div>
                </div>
            `;
        } else {
