<x-app-layout>
    ...
    <div class="py-10 px-6 max-w-4xl mx-auto">
        ...
        <form action="{{ route('meetings.store.proposal') }}" method="POST" class="space-y-6 bg-white p-6 shadow rounded">
            @csrf
            ...
            <div>
                ...
                @error('attendees')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
                <p class="text-sm text-gray-600 mt-1">Hold Ctrl/Cmd to select multiple partners</p>
            </div>

            {{-- Browser timezone --}}
            <input type="hidden" name="tz" id="meeting-tz-proposal" value="">
            <script>
            (function(){
                var el = document.getElementById('meeting-tz-proposal');
                ...
                <div class="flex">
                    <div class="ml-3">
                        <p class="text-sm text-yellow-700">
                            <strong>Note:</strong> This is a meeting proposal. An admin will review and approve it before it appears on the calendar and invitations are sent.
                        </p>
                    </div>
                </div>
            </div>
